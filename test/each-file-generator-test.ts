...
import {SeparatedFileGenerator} from "../src/separated-file-generator";
import {
    EMPTY_PATH,
    EMPTY_PATH_RESULT,
...
describe('Each File Generator', () => {
    describe('Generate', () => {
        ...
        let filegen = new SeparatedFileGenerator(parser)
        it('Should write multiple codes on file', () => {
            filegen.generate(MULTIPLE_ENTRIES_PATH)
            expect(fileExtractor.getExtractedContent(MULTIPLE_ENTRIES_PATH_RESULT)).to.equal(multipleEntriesResult);
        });
        it('Should write no codes on file', () => {
            filegen.generate(EMPTY_PATH)
            expect(fileExtractor.getExtractedContent(EMPTY_PATH_RESULT)).to.equal(emptyResult);
        });
        it('Should write one code on file', () => {
            filegen.generate(ONE_ENTRY_PATH)
            expect(fileExtractor.getExtractedContent(ONE_ENTRY_PATH_RESULT)).to.equal(oneEntryResult);
        });
        it('Should write one false code on file', () => {
            filegen.generate(ONE_FALSE_ENTRY_PATH)
            expect(fileExtractor.getExtractedContent(ONE_FALSE_ENTRY_PATH_RESULT)).to.equal(oneFalseEntryResult);
        });
        it('Should write two codes on file', () => {
            filegen.generate(TWO_ENTRIES_PATH)
            expect(fileExtractor.getExtractedContent(TWO_ENTRIES_PATH_RESULT)).to.equal(twoEntriesResult);
        });
        it('Should write two false codes on file', () => {
            filegen.generate(TWO_ENTRIES_WITH_ONE_FALSE_PATH)
            expect(fileExtractor.getExtractedContent(TWO_ENTRIES_WITH_ONE_FALSE_PATH_RESULT)).to.equal(twoFalseEntriesResult);
        });
    })
});
